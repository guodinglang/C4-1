...
//	GPIOD->odt ^= GPIO_PINS_1;
}


/**
  * @brief  exint8 interrupt handler
  * @param  none
  * @retval none
  */
void EXINT9_5_IRQHandler(void)
{
	...
	{
		if(!STROBE && swMess.parallel == false)
		{
			if(!STROBE)
			{
				swMess.buff = gpio_input_data_read(GPIOB);	
				if(swMess.buff == gpio_input_data_read(GPIOB))
				{
